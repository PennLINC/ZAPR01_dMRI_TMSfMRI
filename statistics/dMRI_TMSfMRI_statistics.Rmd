...
vlPFC-amygdala pathway fiber cross section was not associated with TMS-evoked response in the left amygdala, controlling for age and estimated total intracranial volume

```{r}
covs <- data.frame(demographics$age, demographics$eTIV)
pcor.test(fixels$logFC_mean_5, abs(vlPFC.TMS.ER.HO25$left_amygdala), covs, method=c("spearman"))
```

Sensitivity Analyses

* FD-evoked response association controlling for age and estimated total intracranial volume
```{r}
covs <- data.frame(demographics$age, demographics$eTIV)
