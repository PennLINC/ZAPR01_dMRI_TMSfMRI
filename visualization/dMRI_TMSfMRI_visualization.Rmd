...
sumld<- ddply(datal, ~Region, summarise, mean = mean(abs(ER)), median = median(abs(ER)), lower = lb(abs(ER)), upper = ub(abs(ER)))
...
geom_flat_violin(position = position_nudge(x = .25, y = 0), alpha = .99, fill = c("#3f55a5")) +
geom_point(aes(y=abs(ER), fill=sign, color=sign), pch = 21, position = position_jitter(width = .15), size = 3.15, stroke = .5, alpha = .85) +
geom_boxplot(width = .13, outlier.shape = NA, alpha = 0.5, fill = c("#3f55a5")) +
...
sumld<- ddply(datal, ~Region, summarise, mean = mean(ERdiff), median = median(ERdiff), lower = lb(ERdiff), upper = ub(ERdiff))
